...
export default function CategoryPage({ params }: Props) {
  ...
  return <div className="min-h-screen bg-gray-50"><Header /><main><JsonLd data={{ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: breadcrumbs.map((item, position) => ({ "@type": "ListItem", position: position + 1, name: item.label, item: item.href ?? canonical })) }} /><JsonLd data={{ "@context": "https://schema.org", "@type": "CollectionPage", name: category.heading, description: category.description, url: canonical, inLanguage: "pt-BR" }} />
    <section className="bg-white py-16"><div className="mx-auto max-w-5xl px-4 sm:px-6"><BlogBreadcrumbs items={breadcrumbs} /><h1 className="mt-8 text-4xl tracking-tight text-gray-900 md:text-5xl">{category.heading}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-gray-600">{category.intro}</p><ul className="mt-8 flex flex-wrap gap-2">{category.themes.map((theme) => <li key={theme} className="rounded-full bg-primary/10 px-4 py-2 text-sm text-primary">{theme}</li>)}</ul></div></section>
    <section className="py-16"><div className="mx-auto max-w-5xl px-4 sm:px-6"><h2 className="text-2xl font-semibold text-gray-900">Artigos publicados</h2><div className="mt-8 grid gap-6">{articles.length ? articles.map((article) => <ArticleCard key={article.slug} article={article} category={category} />) : <p className="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-gray-600">Ainda não há artigos publicados nesta categoria. Novos conteúdos serão adicionados em breve.</p>}</div><div className="mt-16"><BlogCta category={category} /></div><nav aria-label="Outras categorias" className="mt-12"><h2 className="text-lg font-semibold text-gray-900">Explore também</h2><div className="mt-4 flex flex-wrap gap-3">{BLOG_CATEGORIES.filter(({ slug }) => slug !== category.slug).map((item) => <LinkChip key={item.slug} href={`/pt/${item.slug}`} label={item.name} />)}</div></nav></div></section>
  </main><Footer /></div>;
}
